...
def run_segmentation():
    ...
    if latest_upload is not None:
        ...
        infer = YOLO("segmentation.pt FOLDER IN HERE")
        result = infer.predict(latest_image_path, save=True)

        segment_dir = os.path.join(settings.BASE_DIR, 'runs', 'segment')
        predict_dirs = [d for d in os.listdir(segment_dir) if
                        os.path.isdir(os.path.join(segment_dir, d)) and d.startswith('predict')]
        ...
        if latest_upload is not None:
            ...
            prediction_files = os.listdir(os.path.join(segment_dir, latest_predict_dir))


            for file_name in prediction_files:
                with open(os.path.join(segment_dir, latest_predict_dir, file_name), 'rb') as image_file:
